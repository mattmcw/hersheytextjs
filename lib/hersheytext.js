 ...
 *    weight {int}: Weight of stroke
 *
 * @returns {string|boolean}
 *   Internal SVG content to be rendered as you see fit.
 *   Returns === false if error.
 */
function renderTextSVG (text, rawOptions = {}) {
  const options = {
    id: 'text',
    font: 'futural',
    charSpacingAdjust: 0,
  ...
  try {
    ...
    const lineHeight = options.lineHeight || parseInt(font.info['units-per-em'], 10);
    const multiplyer = font.type === 'svg' ? 1 : 1.68;
    const offset = { left: 0, top: parseInt(font.info['units-per-em'], 10) };
    ...
    for (let text of lines) {
      $groupLines[lineCount] = $('<g>').attr('id', options.id + '-line-' + lineCount);
      $textGroup.append($groupLines[lineCount]);
      ...
      for(let w in words) {
        ...
        for(let i in word) {
          ...
          if (char) {
            ...
            if (font.type === 'svg') {
              $path.attr('transform', `translate(${offset.left}, ${offset.top}) scale(1, -1)`);
            }

            // Add the char to the DOM group.
            $groupLines[lineCount].append($path);
...
exports.renderTextSVG = renderTextSVG;
