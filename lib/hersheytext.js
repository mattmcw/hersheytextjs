...
function renderTextSVG (text, rawOptions = {}) {
  if (text.indexOf('\n') !== -1) {
    return renderMultilineSVG(text, rawOptions);
  }
  const options = {
    id: 'text',
    font: 'futural',
...
function renderMultilineSVG (text, rawOptions = {}) {
  ...
  for (let line of lines) {
    lineOptions = {
      ...options,
      id : `${options.id}-line${lineCount}`
    }
    lineText = renderTextSVG(line, lineOptions);
    if (lineText) {
      linesText += lineText + '\n';
    }
    lineCount++;
    options.pos.y += options.lineHeight;
  }
  return `${group}${linesText}${groupClose}`;
}

exports.renderMultilineSVG = renderMultilineSVG
